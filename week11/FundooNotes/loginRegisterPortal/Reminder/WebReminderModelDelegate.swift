...
extension ReminderModel{
    ...
    func getReminderedNotesFromServer(completion: @escaping ((_ noteList: [oneNote]) -> Void)) {
        let access_token: String = UserDefaults.standard.string(forKey: "accessToken")!
        let url: String = WebServicesCommonData.getReminderNotesListURL + "?access_token=" + access_token
                    ...
                    for i in 0..<secondData.count {
                        ...
                        while j < CommonUIController.arrayOfColors.count && colorFoundFlag == false{
                            ...
                            if UIColor().compareUIColors(color1,color2) {
                                ...
                                GetData.arrayOfReminderedNotes.append(oneNote(noteTitle,noteDescription,j+1,indexx,noteArchiveStatus,noteImportantStatus,noteReminderInfoString))
